...
io.on('connection', (socket) => {
    ...
    socket.on('join', (options, callback) => {
        ...
        socket.emit('send', generateMessage('System', 'Welcome!'));
        socket.broadcast.to(user.room).emit('send', generateMessage('System',`${user.username} has joined!`))
    ...
    socket.on('disconnect', () => {
        ...
        if(user)
        {
            io.to(user.room).emit('send', generateMessage('System' ,`${user.username} has left`))
            io.to(user.room).emit('roomData', {
                room: user.room,
                users: getUsersInRomm(user.room)
            })
